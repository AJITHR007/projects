...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['requestId'])) {
    ...
    $updateRequest = $conn->prepare("UPDATE compensation SET status = 'pending' WHERE id = ?");
    ...
    if ($updateRequest->rowCount() > 0) {
        ...
        $mail->Password = 'kwhf znks pfrv dnzw';  // Replace with your SMTP password
        $mail->SMTPSecure = 'tls';
        $mail->Port = 587;
        $mail->setFrom('umaatc@gmail.com', 'Srays');  // Replace with your email and name
        $mail->addAddress($requestDetails['email'], $requestDetails['empName']);  // Use user's email and name
        $mail->Subject = 'Your Compensation Request is Pending';
        $mail->Body = 'Dear ' . $requestDetails['empName'] . ',<br>Your compensation request is pending.';

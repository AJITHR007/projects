...
if (isset($_POST['resetEmail'])) {
    ...
    $query = "SELECT * FROM recordadd WHERE email = :email AND usertype = 'admin'";
    ...
    if ($result == 0) {
        echo "1"; // Email not found
        echo "<script>alert('Email Not Found or Not Admin!'); window.location='adminlogin.php';</script>";
        ...
        if (registeredEmail($email, $token) && $stmt->execute()) {
            echo "<script>alert('Email Sent Successfully!'); window.location='adminlogin.php';</script>";
            // echo "2"; // Email sent successfully
        } else {
            echo "<script>alert('Email not Sent!!'); window.location='adminlogin.php';</script>";
